...
function isLeapYear(date) {
  ...
  if (year % 4) res = false;
  else if (year % 100) res = true;
  else if (year % 400) res = false;
  else res = true;
...
function angleBetweenClockHands(date) {
  ...
  const minsSince12 = (60 * hours) + minutes;

  const hourAngle = 0.5 * minsSince12;
  const minuteAngle = 6 * minutes;

  let degAngle = Math.abs(hourAngle - minuteAngle);
  if (degAngle > 180) degAngle %= 180;
